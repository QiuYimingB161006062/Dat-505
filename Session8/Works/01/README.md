...
#### Description ####
Set up 5 eyes and let them look at the mouse.
...
* Calculate the range and position of eyes' rotation and set the range.
...
function render() {
	for (var i = 0; i < eyesNum; i++) {
		...
		if (mouseX<600) eyes[2].rotation.y = map_range(mouseX, 0, , -1.14, -0.75);
		else eyes[2].rotation.y = map_range(mouseX, 600, window.innerWidth,  -0.75, 0.2);
		...
		if (mouseX<600) eyes[4].rotation.y = map_range(mouseX, 0, 600, -1.14, -0.75);
		else eyes[4].rotation.y = map_range(mouseX, 600, window.innerWidth, -0.75, 0.2);
